...
try {
    ...
    try {
        if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_REPORTING)) {
            $total_records = $DB->count_records(\local_alx_report_api\constants::TABLE_REPORTING, ['is_deleted' => 0]); // Only active records
                        ...
                        if ($api_analytics['summary']['total_calls'] > 0) {
                            ...
                            if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_LOGS)) {
                                ...
                                if (isset($table_info['error_message'])) {
                                    // Use standard Moodle field name
                                    $time_field = 'timecreated';
                                    $total_calls = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, "{$time_field} >= ?", [time() - 86400]);
                                    $error_calls = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, 
                ...
                <div class="dashboard-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; border: none;">
                    ...
                    <div class="card-body">
                        ...
                        if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_SYNC_STATUS)) {
                            $total_sync_entries = $DB->count_records(\local_alx_report_api\constants::TABLE_SYNC_STATUS);
                            $recent_syncs = $DB->count_records_select(\local_alx_report_api\constants::TABLE_SYNC_STATUS, 
                                'last_sync_timestamp > ?', [time() - 86400]);
                            // Fetch the most recent sync timestamp
                            $last_sync_timestamp = $DB->get_field_sql('SELECT MAX(last_sync_timestamp) FROM {' . \local_alx_report_api\constants::TABLE_SYNC_STATUS . '}');
                        } else {
                        ...
                        if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_REPORTING)) {
                            ...
                            foreach ($companies_list as $comp) {
                                $record_count = $DB->count_records(\local_alx_report_api\constants::TABLE_REPORTING, [
                                    'companyid' => $comp->id,
                                    'is_deleted' => 0
                                ]);
                        ...
                        foreach ($sync_settings as $setting) {
                            if ($setting === 'sync_mode') {
                                ...
                                if ($value !== '' && $value !== null) {
                                    ...
                                    $DB->delete_records(\local_alx_report_api\constants::TABLE_SETTINGS, [
                                        'companyid' => $companyid,
    ...
    <div id="monitoring-tab" class="tab-content">
        ...
        try {
            ...
            if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_REPORTING)) {
                ...
                $sample_query = $DB->get_records(\local_alx_report_api\constants::TABLE_REPORTING, [], '', 'id', 0, 1);
        ...
        if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_LOGS)) {
            // Use standard Moodle field name
            $time_field = 'timecreated';
            $last_hour_calls = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, 
                ...
                const apiCalls = [<?php 
                    ...
                    if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_LOGS)) {
                        ...
                        for ($i = 0; $i < 24; $i++) {
                            ...
                            $count = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, 
