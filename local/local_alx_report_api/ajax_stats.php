...
try {
    ...
    if ($DB->get_manager()->table_exists(constants::TABLE_REPORTING)) {
        ...
        if (isset($table_info['is_deleted'])) {
            $stats['total_records'] = $DB->count_records(constants::TABLE_REPORTING, ['is_deleted' => 0]);
        } else {
    ...
    if ($DB->get_manager()->table_exists(constants::TABLE_LOGS)) {
        ...
        $stats['api_calls_today'] = $DB->count_records_select(constants::TABLE_LOGS, "{$time_field} >= ?", [$today_start]);
    }
