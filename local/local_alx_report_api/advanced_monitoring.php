            ...
            try {
                ...
                try {
                    if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_REPORTING)) {
                        $reporting_count = $DB->count_records(\local_alx_report_api\constants::TABLE_REPORTING);
...
try {
    ...
    if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_LOGS)) {
        ...
        $api_performance['total_calls_24h'] = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, 
            "{$time_field} >= ?", [$last_24h]);
        ...
        if (isset($table_info['status'])) {
            $success_count = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, 
        ...
        if (isset($table_info['response_time'])) {
            $api_performance['timeout_errors'] = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, 
...
if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_LOGS)) {
    ...
    $has_data = $DB->count_records(\local_alx_report_api\constants::TABLE_LOGS) > 0;
    ...
    try {
        ...
        if ($DB->record_exists(\local_alx_report_api\constants::TABLE_SETTINGS, ['companyid' => $company->id])) {
            ...
            if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_LOGS)) {
                ...
                // Get request count
                $company_data['requests_today'] = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, 
...
try {
    ...
    if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_LOGS)) {
        ...
        for ($hour = 0; $hour < 24; $hour++) {
            ...
            $hour_count = $DB->count_records_select(\local_alx_report_api\constants::TABLE_LOGS, 
                "{$time_field} >= ? AND {$time_field} < ?", [$hour_start, $hour_end]);
...
foreach ($company_intelligence as $key => $company) {
    try {
        ...
        if ($DB->get_manager()->table_exists(\local_alx_report_api\constants::TABLE_CACHE) && $company['requests_today'] > 0) {
        ...
        $api_settings = $DB->get_record(\local_alx_report_api\constants::TABLE_SETTINGS, ['companyid' => $company['shortname']]);
        if ($api_settings) {
