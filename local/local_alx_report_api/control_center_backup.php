            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 20px; margin-top: 20px; margin-bottom: 20px;">
                ...
                <div class="dashboard-card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; margin-bottom: 0;">
                    ...
                    <div class="card-body">
                        ...
                        // Calculate real response time and success rate from actual data
                        $response_time = 0;
                        $success_rate = 0;
                        $has_api_data = false;
                        ...
                        if ($api_analytics['summary']['total_calls'] > 0) {
                            $has_api_data = true;
                            // Calculate average response time from logs if available
                            if ($DB->get_manager()->table_exists('local_alx_api_logs')) {
                                ...
                                ", [time() - 86400]);
                                $response_time = $avg_response ? round($avg_response / 1000, 2) : 0; // Convert to seconds
                            }
                            
                            // Calculate success rate from logs
                            if ($DB->get_manager()->table_exists('local_alx_api_logs')) {
                                $total_calls = $DB->count_records_select('local_alx_api_logs', 'timecreated >= ?', [time() - 86400]);
                                $error_calls = $DB->count_records_select('local_alx_api_logs', 'timecreated >= ? AND error_message IS NOT NULL AND error_message != ?', [time() - 86400, '']);
                                if ($total_calls > 0) {
                        ...
                        <div style="margin-bottom: 20px;">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                ...
                                <span style="color: #4ade80; font-weight: 600;"><?php echo $response_time > 0 ? $response_time . 's avg' : 'N/A'; ?></span>
                            </div>
                            <div style="background: rgba(255,255,255,0.2); border-radius: 10px; height: 8px; overflow: hidden;">
                                <?php 
                                $response_percentage = $response_time > 0 ? min(100, (5 - $response_time) / 5 * 100) : 0; // Good if under 5s
                                ?>
